...
JNIMediaPlayerListener::JNIMediaPlayerListener(JNIEnv *env, jobject thiz, jobject weak_thiz) {
    ...
    if (clazz == NULL) {
        LOGI("Can't find wseemann/media/FFmpegMediaPlayer");
...
wseemann_media_FFmpegMediaPlayer_setDataSourceAndHeaders(
    ...
    if (keys && values != NULL) {
        ...
        if (keysCount != valuesCount) {
            LOGI("keys and values arrays have different length");
            jniThrowException(env, "java/lang/IllegalArgumentException", NULL);
            return;
        }
...
//    LOGI("setAudioStreamType: %d", streamtype);
    MediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return;
...
wseemann_media_FFmpegMediaPlayer_setVolume(JNIEnv *env, jobject thiz, float leftVolume, float rightVolume) {
    LOGI("setVolume: left %f  right %f", leftVolume, rightVolume);
    MediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return;
...
static void wseemann_media_FFmpegMediaPlayer_set_audio_session_id(JNIEnv *env, jobject thiz, jint sessionId) {
    LOGI("set_session_id(): %d", sessionId);
    MediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return;
...
wseemann_media_FFmpegMediaPlayer_setAuxEffectSendLevel(JNIEnv *env, jobject thiz, jfloat level) {
    LOGI("setAuxEffectSendLevel: level %f", level);
    MediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return;
...
static void wseemann_media_FFmpegMediaPlayer_attachAuxEffect(JNIEnv *env, jobject thiz, jint effectId) {
    LOGI("attachAuxEffect(): %d", effectId);
    MediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return;
...
static int register_wseemann_media_FFmpegMediaPlayer(JNIEnv *env) {
    ...
    if (clazz == NULL) {
        LOGI("Native registration unable to find class 'wseemann/media/FFmpegMediaPlayer'");
        return JNI_ERR;
    }
    if (env->RegisterNatives(clazz, gMethods, numMethods) < 0) {
        LOGI("RegisterNatives failed for 'wseemann/media/FFmpegMediaPlayer'");
        return JNI_ERR;
    }
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if (register_wseemann_media_FFmpegMediaPlayer(env) < 0) {
        LOGI("ERROR: FFmpegMediaPlayer native registration failed\n");
        goto bail;
    }
