...
public class EventBus {
    ...
    EventBus(EventBusBuilder builder) {
        ...
        subscriberMethodFinder =
                new SubscriberMethodFinder(builder.subscriberInfoIndexes, builder.strictMethodVerification, builder.ignoreGeneratedIndex);
     ...
     * event of an event's type is kept in memory for future access by subscribers using {@link Subscribe#sticky()}.
     */
    public void postSticky(Class clazz, Object event) {
        synchronized (stickyEvents) {
            stickyEvents.put(clazz, event);
        }
        // Should be posted after it is putted, in case the subscriber wants to remove immediately
        post(event);
    }
        ...
        if (subscriberMethod.sticky) {
            if (eventInheritance) {
                ...
                for (Map.Entry<Class<?>, Object> entry : entries) {
                    ...
                    Class<?> candidateEventType2 = entry.getValue().getClass();
                    if (eventType.isAssignableFrom(candidateEventType) || eventType.isAssignableFrom(candidateEventType2)) {
