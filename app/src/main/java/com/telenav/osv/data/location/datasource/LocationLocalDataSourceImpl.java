...
public class LocationLocalDataSourceImpl implements LocationLocalDataSource {
    ...
    private LocationLocalDataSourceImpl(@NonNull Context context) {
        locationDao = Injection.provideOSCDatabase(context).locationDao();
    }
    ...
    @Override
    public boolean saveLocation(@NonNull OSVLocation osvLocation, String videoID, String frameID) {
        boolean locationSaved = locationDao.insert(DataConverter.toLocationEntity(osvLocation, videoID, frameID)) != 0;
        Log.d(TAG, String.format("Location persisted. Status: %s. Id: %s", locationSaved, osvLocation.getID()));
    ...
    @Override
    public Maybe<List<OSVLocation>> getLocationsBySequenceId(String sequenceID) {
        return locationDao
                ...
                .flatMap(entities -> Observable
                        ...
                        .map(DataConverter::toOSVLocation)
                        .toList()
                        .toMaybe());
    }

    @Override
    public Single<List<OSVLocation>> getLocations() {
        return locationDao
                .findAll()
    ...
    @Override
    public Maybe<List<OSVLocation>> getLocationsByVideoId(String videoID) {
        return locationDao
    ...
    @Override
    public Maybe<OSVLocation> getLocationById(String locationID) {
        return locationDao
                .findByID(locationID)
    ...
    @Override
    public Maybe<OSVLocation> getLocationByFrameId(String frameID) {
        return locationDao
