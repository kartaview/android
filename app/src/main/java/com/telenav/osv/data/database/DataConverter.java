...
public class DataConverter {
    ...
    /**
     * @param osvLocation the custom {@code location} model to be translated into a {@code LocationEntity}.
     ...
     */
    public static LocationEntity toLocationEntity(OSVLocation osvLocation, @Nullable String videoID, @Nullable String frameID) {
        Location location = osvLocation.getLocation();
        return new LocationEntity(
                osvLocation.getID(),
                location.getLatitude(),
                location.getLongitude(),
                osvLocation.getSequenceId(),
                videoID,
                frameID);
    }
    ...
    /**
     * @param osvLocations {@code collection} of {@code OSVLocation} objects.
     * @return {@code collection} of {@code Android} locations translated from {@code collection} of {@code OSVLocation} objects.
     */
    public static List<Location> toLocations(@NonNull List<OSVLocation> osvLocations) {
        List<Location> locations = new ArrayList<>();
        if (!osvLocations.isEmpty()) {
            for (OSVLocation osvLocation : osvLocations) {
                locations.add(osvLocation.getLocation());
            }
        }
        return locations;
    }
    ...
    /**
     * @param locationEntity the {@code LocationEntity} to be translated into an {@code OSVLocation}.
     * @return {@code OSVLocation} model with all the data from the given params.
     */
    public static OSVLocation toOSVLocation(@NonNull LocationEntity locationEntity) {
        Location location = new Location(StringUtils.EMPTY_STRING);
        location.setLatitude(locationEntity.getLatitude());
        location.setLongitude(locationEntity.getLongitude());
        return new OSVLocation(locationEntity.getLocationId(), location, locationEntity.getSequenceID());
    }
    ...
    public static LocalSequence toLocalSequence(@NonNull SequenceEntity sequenceEntity) {
        ...
        SequenceDetailsLocal sequenceDetailsLocal = new SequenceDetailsLocal(
                new OSVFile(sequenceEntity.getFilePath()),
