...
public class VideoSaver extends DataSaver {
    ...
    @Override
    public void saveFrame(final byte[] jpegData, final Location mLocationF, final float mAccuracyF, final int mOrientationF, final long mTimestampF) {
        ...
        mBackgroundHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    SequenceDB.instance.insertVideoIfNotAdded(mSequence.getId(), mVideoIndexF, mSequence.getFolder().getPath() + "/" + mVideoIndexF + ".mp4");
                } catch (Exception ignored) {}

                try {
                    SequenceDB.instance.insertPhoto(mSequence.getId(), mVideoIndexF, mIndexF, mSequence.getFolder().getPath() + "/" + mVideoIndexF + ".mp4", mLocationF
                                    .getLatitude(),
