...
public class IssueReporter extends NetworkManager implements RequestQueue.RequestFinishedListener<Object> {
    ...
    public void createIssue(final NetworkResponseDataListener<IssueData> listener, String description) {
        mHolderQueue.clear();
        mListener = listener;
        IssueCreationRequest issueRequest = new IssueCreationRequest(URL_ISSUE_CREATE, new OsvRequestResponseListener<IssueCreationParser, IssueData>(mIsueCreationParser) {
    ...
    private void uploadIssueFile(final NetworkResponseDataListener<IssueData> listener, final int id, final OSVFile file, final int index) {
        ...
        final IssueUploadRequest request = new IssueUploadRequest(URL_ISSUE_UPLOAD, new OsvRequestResponseListener<HttpResponseParser, ApiResponse>(mHttpResponseParser) {
