...
//    }
    ...
    private static boolean pointIsInBB(SKBoundingBox bb, double lat, double lon) {
        return bb != null && (bb.getTopLeft().getLatitude() >= lat && lat >= bb.getBottomRight().getLatitude() && bb.getTopLeft().getLongitude() <= lon && lon <= bb
        ...
        if (assessBBDifference(mTriggerBB, smallBB) && !requestSent && mGeometryRetriever != null) {
            ...
            BackgroundThreadPool.post(new Runnable() {
                ...
                public void run() {
                    ...
                    }, requestedBB.getTopLeft().getLatitude() + "," + requestedBB.getTopLeft().getLongitude(), requestedBB.getBottomRight().getLatitude() + ","
                            + requestedBB.getBottomRight().getLongitude(), MAX_RESOLUTION_ZOOM);
                }
            });
        }
    }
