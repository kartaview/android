...
public class SafePlaybackManager extends PlaybackManager implements SeekBar.OnSeekBarChangeListener {
    ...
    public SafePlaybackManager(OSVActivity act, LocalSequence sequence, FrameLocalDataSource frameLocalDataSource) {
        activity = act;
        mSequence = sequence;
                        ...
                        frames -> {
                            ...
                            for (Frame frame : frames) {
                                ...
                                if (frameLocation != null && frameLocation.getLatitude() != 0.0 && frameLocation.getLongitude() != 0.0) {
                                    ...
                                    ImageCoordinate coord = new ImageCoordinate(frameLocation.getLatitude(), frameLocation.getLongitude(), frameSeqIndex);
                                    nodes.add(new ImageFile(new OSVFile(frame.getFilePath()), frameSeqIndex, coord, false));
                                    track.add(coord);
                                }
                            }
