...
public abstract class PlaybackManager {
    ...
    public static PlaybackManager get(OSVActivity activity, Sequence sequence, FrameLocalDataSource frameLocalDataSource, VideoLocalDataSource videoLocalDataSource) {
        if (sequence.getType() == Sequence.SequenceTypes.LOCAL) {
            ...
            return sequence.getCompressionDetails() instanceof SequenceDetailsCompressionJpeg ?
                    new SafePlaybackManager(activity, localSequence, frameLocalDataSource) :
