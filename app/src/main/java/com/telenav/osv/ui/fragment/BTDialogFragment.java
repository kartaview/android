    ...
    private OnDeviceSelectedListener mDeviceClickListener = new OnDeviceSelectedListener() {
        ...
        public void onDeviceSelected(BluetoothDevice device) {
            if (device != null) {
                ...
                preferences.edit()
                        .putString(mObdManager.getType() == ObdManager.TYPE_BLE ? Constants.EXTRAS_BLE_DEVICE_ADDRESS : Constants.EXTRAS_BT_DEVICE_ADDRESS, device.getAddress())
                        .apply();
...
//        mNewDevicesListView.setOnItemClickListener(mDeviceClickListener);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            mDevicesAdapter.highlight(mObdManager.getType() == ObdManager.TYPE_BLE ? BluetoothDevice.DEVICE_TYPE_LE : BluetoothDevice.DEVICE_TYPE_CLASSIC);
        }
