 ...
 */
public class RecordingViewModel extends AndroidViewModel implements RecordingContract.CameraControlsView, NetworkBroadcastReceiver.NetworkConnectionListener {
    ...
    private static final String FORMAT_CURRENCY_ESTIMATION_SUFFIX = " (Est.) ";

    private static final String FORMAT_SLASH_SEPARATOR = " / ";
    ...
    /**
     * {@code BroadcastReceiver} used to listen for network connection.
     */
    ...
    public RecordingViewModel(@NonNull Application application) {
        super(application);
        ApplicationPreferences appPrefs = ((OSVApplication) application).getAppPrefs();
        OSVApplication osvApplication = (OSVApplication) application;
        presenter = new RecordingPresenterImpl(application.getApplicationContext(), appPrefs,
                osvApplication.getRecorder(),
                osvApplication.getScore(),
                osvApplication.getRecordingPersistence(),
                Injection.provideLocationService(application.getApplicationContext()),
                new PayRateInteractorImpl(appPrefs.getIntPreference(PreferenceTypes.K_DEBUG_SERVER_TYPE), appPrefs.getStringPreference(PreferenceTypes.K_ACCESS_TOKEN)),
                this);
        networkBroadcastReceiver = new NetworkBroadcastReceiver();
        networkBroadcastReceiver.addNetworkConnectionListener(this);
    }
    ...
    public void onScoreChanged(ScoreChangedEvent scoreChangedEvent) {
        ...
        String multiplier = String.valueOf(scoreChangedEvent.getMultiplier()) + FORMAT_COVERAGE_MULTIPLIER_SUFFIX;
        scoreMultiplierObservable.setValue(multiplier);
        scorePointsObservable.setValue(String.valueOf(scoreChangedEvent.getScore()));
    }

    @Override
    public void onPaymentChanged(ByodDriverPayRateUpdatedEvent byodDriverPayRateUpdatedEvent) {
        if (scorePointsObservable != null) {
            if (byodDriverPayRateUpdatedEvent.getPayRateValue() != ScoreManager.UNKNOWN_VALUE) {
                scorePointsObservable.setValue(byodDriverPayRateUpdatedEvent.getCurrency() +
                        FormatUtils.getDistanceUnitLabel(presenter.isImperial(), FORMAT_SLASH_SEPARATOR, false) + FORMAT_CURRENCY_ESTIMATION_SUFFIX);
                scoreMultiplierObservable.setValue(String.valueOf(byodDriverPayRateUpdatedEvent.getPayRateValue()));
            }
        }
    }
    ...
    public void onLocationAccuracyChanged(int accuracyType) {
        ...
        switch (accuracyType) {
            ...
            case AccuracyType.ACCURACY_MEDIUM:
                ...
                break;
            case AccuracyType.ACCURACY_GOOD:
    ...
    public boolean isTaggingEnabled() {
        return presenter.isTaggingFeatureEnabled();
    }
    ...
    protected void onCleared() {
        ...
        networkBroadcastReceiver.removeNetworkConnectionListener();
        presenter.release();
    }
    ...
    public void onNetworkConnectionChange(boolean isConnected) {
        if (isConnected) {
            presenter.requestPayRateData();
        }
    }

    public boolean isByodDriver() {
        return presenter.isByodDriver();
    }
    ...
    public void onStart() {
        ...
        getApplication().registerReceiver(networkBroadcastReceiver, new IntentFilter(ConnectivityManager.CONNECTIVITY_ACTION));
    }

    /**
     * Method defining the logic for the {@code onStart} method from view's lifecycle event for tagging fragment.
     */
    ...
    public void onStop() {
        ...
        getApplication().unregisterReceiver(networkBroadcastReceiver);
    }
    ...
    public void restoreInstanceState(Bundle savedInstanceState) {
        ...
        if (isScoreVisible()) {
            ...
            if (!presenter.isByodDriver() && scorePointsObservable.getValue() != null) {
    ...
    public boolean isTaggingMode() {
        return presenter.isTaggingMode();
    }

    /**
     * Sets the tagging mode, representing if the tagging screen is visible or not.
     * @param isTaggingModeEnable {@code true} if the tagging screen is visible, {@code false} otherwise.
     */
    public void setTaggingMode(boolean isTaggingModeEnable) {
        presenter.setTaggingMode(isTaggingModeEnable);
    }

    private boolean isScoreVisible() {
        return presenter.isByodDriver() || presenter.isGamificationEnable();
    }
