...
public class CameraPreviewPresenterImpl implements CameraPreviewPresenter, TextureViewPreview.SurfaceChangedListener {
    ...
    public void openCamera(CompletableObserver observer) {
        ...
        if (appPrefs.getStringPreference(PreferenceTypes.K_USED_CAMERA_ID, "").isEmpty()) {
            double maxFov = 0;
            String cameraId = "";
            for (CameraDeviceData cameraDeviceData : camera.getCameraDevices()) {
                if (cameraDeviceData.getHorizontalFOV() > maxFov) {
                    maxFov = cameraDeviceData.getHorizontalFOV();
                    cameraId = cameraDeviceData.getCameraId();
                }
            }
            appPrefs.saveStringPreference(PreferenceTypes.K_USED_CAMERA_ID, cameraId);
        }
        camera.openCamera(appPrefs.getStringPreference(PreferenceTypes.K_USED_CAMERA_ID))
