 ...
 */
public class CameraPreviewFragment extends OSCBaseFragment implements ObdContract.PermissionsListener {
    ...
    /**
     * Instance to the {@link GestureDetector} to detect the gestures over the camera preview.
     */
    private GestureDetector gestureDetector;

    /**
     * Listener for camera preview touch events.
     */
    private View.OnTouchListener previewTouchListener = new View.OnTouchListener() {
    ...
    private void observeOnCameraPermissionEvent() {
        viewModel.getCameraPermissionsObservable().observe(this, permissions -> {
            if (getActivity() != null && permissions != null) {
                ActivityCompat.requestPermissions(getActivity(), permissions, OSVApplication.CAMERA_PERMISSION);
            }
        });
    }
