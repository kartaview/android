 ...
 */
public class ObservableMergeList<T> extends AbstractList<T> implements ObservableList<T> {

  private final ArrayList<List<? extends T>> lists = new ArrayList<>();

  private final ObservableMergeList.ListChangeCallback
      callback = new ObservableMergeList.ListChangeCallback();
   ...
   */
  public ObservableMergeList<T> insertItem(T object) {
  ...
  public ObservableMergeList<T> insertList(@NonNull ObservableList<? extends T> list) {
   ...
   * Removes the given item from the merge list.
   */
  public boolean removeItemSilent(T object) {
    int size = 0;
    for (int i = 0, listsSize = lists.size(); i < listsSize; i++) {
      List<? extends T> list = lists.get(i);
      if (!(list instanceof ObservableList)) {
  ...
  public boolean addAll(ObservableList<? extends T> c) {
    for (int i2 = 0, listS = c.size(); i2 < listS; i2++) {
      T item = c.get(i2);
      insertItem(item);
    }
    return false;
  ...
  public boolean removeAll(ObservableList<? extends T> c) {
    for (int i2 = 0, listS = c.size(); i2 < listS; i2++) {
      T item = c.get(i2);
      removeItem(item);
    }
  ...
  class ListChangeCallback extends OnListChangedCallback {
    ...
    public void onChanged(ObservableList sender) {
      modCount += 1;
      listeners.notifyChanged(ObservableMergeList.this);
    }
    ...
    public void onItemRangeChanged(ObservableList sender, int positionStart, int itemCount) {
      ...
      for (int i = 0, listsSize = lists.size(); i < listsSize; i++) {
        ...
        if (list == sender) {
          listeners.notifyChanged(ObservableMergeList.this, size + positionStart, itemCount);
          return;
        }
        size += list.size();
      }
    ...
    public void onItemRangeInserted(ObservableList sender, int positionStart, int itemCount) {
      ...
      for (int i = 0, listsSize = lists.size(); i < listsSize; i++) {
        List list = lists.get(i);
        if (list == sender) {
          listeners.notifyInserted(ObservableMergeList.this, size + positionStart, itemCount);
          return;
        }
        size += list.size();
      }
    ...
    public void onItemRangeMoved(ObservableList sender, int fromPosition, int toPosition, int itemCount) {
      ...
      for (int i = 0, listsSize = lists.size(); i < listsSize; i++) {
        List list = lists.get(i);
        if (list == sender) {
          listeners.notifyMoved(ObservableMergeList.this, size + fromPosition, size + toPosition, itemCount);
          return;
        }
        size += list.size();
      }
    ...
    public void onItemRangeRemoved(ObservableList sender, int positionStart, int itemCount) {
      ...
      for (int i = 0, listsSize = lists.size(); i < listsSize; i++) {
        List list = lists.get(i);
        if (list == sender) {
          listeners.notifyRemoved(ObservableMergeList.this, size + positionStart, itemCount);
          return;
        }
        size += list.size();
      }
