...
class ScreenDecorator {
    ...
    private View.OnClickListener loginOnClickListener = v -> initLogin();
    ...
    public void onUserTypeChanged(UserTypeChangedEvent event) {
        ...
        MenuItem item = navigationView.getMenu().findItem(R.id.menu_leader_board);
        ...
        if (event.logged) {
            ...
            if (mUserPhotoImageView != null) {
                if (event.accountData.getProfilePictureUrl() != null && event.accountData.getProfilePictureUrl().length() > 0) {
                    ...
                    Glide.with(activity).load(NetworkUtils.provideGlideUrlWithAuthorizationIfRequired(appPrefs, event.accountData.getProfilePictureUrl())).centerCrop().dontAnimate()
    ...
    void onScreenChanged() {
        ...
        switch (getCurrentScreen()) {
            case ScreenComposer.SCREEN_MAP:
                ...
                statusBarColor = R.color.default_purple;
                        ...
                        .subscribe(
                                ...
                                user -> {
                                    ...
                                    if (userPhoto.length() > 0) {
                                        ...
                                        Glide.with(activity).load(NetworkUtils.provideGlideUrlWithAuthorizationIfRequired(appPrefs, userPhoto)).centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL).skipMemoryCache(false).dontAnimate()
                                                .placeholder(ResourcesCompat.getDrawable(activity.getResources(), R.drawable.vector_profile_placeholder, null))
                                                .error(ResourcesCompat.getDrawable(activity.getResources(), R.drawable.vector_profile_placeholder, null))
                                                .listener(MainActivity.mGlideRequestListener).into(mUserPhotoImageView);
                                    }
