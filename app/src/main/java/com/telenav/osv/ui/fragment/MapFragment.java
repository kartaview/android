...
public class MapFragment extends DisplayFragment implements SKMapSurfaceListener, PlaybackManager.PlaybackListener {
    ...
    private Runnable mDisplayLocalSequencesRunnable = new Runnable() {
        @Override
        public void run() {
...
//                    EventBus.postSticky(new GpsCommand(true, true)); todo for power optimisation
    ...
    public void onSurfaceCreated(SKMapViewHolder mapHolder) {
        ...
        mapIntSetting.setFirstLabelOption(SKMapInternationalizationSettings.SKMapInternationalizationOption.MAP_INTERNATIONALIZATION_OPTION_INTL);
        mapIntSetting.setSecondLabelOption(SKMapInternationalizationSettings.SKMapInternationalizationOption.MAP_INTERNATIONALIZATION_OPTION_LOCAL);
    ...
    private void refreshDisplayedSequences() {
        ...
        if (assessBBDifference(lastBoundingBox, bbnormal) || (mLastSegmentsDisplayed != null && zoomForRequest - mLastSegmentsDisplayed.zoom > 1)) {
                    ...
                    if (mapView != null) {
                        ...
                        mGeometryRetriever.listSegments(new NetworkResponseDataListener<GeometryCollection>() {
                            ...
                            public void requestFinished(int status, GeometryCollection collectionData) {
                                if (mCurrentMode == MODE_IDLE) {
                                    EventBus.postSticky(new SegmentsReceivedEvent(collectionData.getSegmentList(), new Object(), false, boundingBox, zoomForRequest));
                                }
                            }
                        }, boundingBox.getTopLeft().getLatitude() + "," + boundingBox.getTopLeft().getLongitude(), boundingBox.getBottomRight().getLatitude() + ","
                                + boundingBox.getBottomRight().getLongitude(), zoomForRequest);
                    }
                }
            };
...
//                segment.setColor(new float[]{0.447f, 0.447f, 0.447f, 1.0f}); grey
                ...
                mMatchedPolyline.setOutlineColor(new float[]{0f / 255f, 125f / 255f, 0f / 255f, 1.0f});  //washed out red, inverse of washed out accent
                mMatchedPolyline.setOutlineSize(3);
                mMatchedPolyline.setOutlineDottedPixelsSolid(50000);
                mMatchedPolyline.setOutlineDottedPixelsSkip(1);
