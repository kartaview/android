 ...
 */
public class CameraControlsFragment extends KVBaseFragment implements ObdContract.PermissionsListener {
    ...
    /**
     * The {@code TextView} representing the points estimation label.
     */
    private TextView textViewPointsValue;

    /**
     * The {@code TextView} representing the multiplier value.
     */
     ...
     */
    private View reportIssueLayout;

    /**
     * Instance to the application preferences in order to check the settings for the app which will affect the UI.
     */
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        this.recordingViewModel = ViewModelProviders.of(getActivity()).get(RecordingViewModel.class);
    ...
    public boolean handleBackPressed() {
        if (!recordingViewModel.isRecording() && getActivity() != null) {
            KVApplication application = (KVApplication) getActivity().getApplication();
    ...
    /**
     * Sets the application prefs.
     */
    private void setAppPrefs() {
        Activity activity = getActivity();
        if (activity != null) {
            appPrefs = ((KVApplication) activity.getApplication()).getAppPrefs();
        }
    }
    ...
    /**
     * Initializes report issue layout to show {@link ReportIssueBottomDialogFragment}
     * This layout should be visible only when the recording is started.
     */
    private void initReportIssue() {
        FragmentActivity activity = getActivity();
        if (activity != null) {
            reportIssueLayout = fragmentLayout.findViewById(R.id.cl_report);
            reportIssueLayout.setOnClickListener(v -> {
                ReportIssueBottomDialogFragment.newInstance().show(
                        activity.getSupportFragmentManager(), ReportIssueBottomDialogFragment.TAG);
            });
        }
    }
    ...
    private void observeOnRecordingPermissionsEvent() {
        recordingViewModel.getRecordingPermissionObservable().observe(this, permissions -> {
            if (getActivity() != null && permissions != null) {
                ActivityCompat.requestPermissions(getActivity(), permissions, KVApplication.START_RECORDING_PERMISSION);
            }
        });
    }
    ...
    private void observeOnObdTooltipEvent() {
        obdViewModel.getObdTooltipObservable().observe(this, tooltipStyle -> {
            ...
            if (obdTooltip != null) {
                ...
                obdTooltip.invalidateTooltip(new KVTooltip.Builder(imageViewObdConnection, (ViewGroup) fragmentLayout, tooltipStyle.getTooltipStyle())
                        .setMessage(tooltipStyle.getTooltipMessage())
                        .setGravity(Gravity.BOTTOM | Gravity.START));
                obdTooltip.show(!animateTooltip);
            } else {
                obdTooltip = new KVTooltip.Builder(imageViewObdConnection, (ViewGroup) fragmentLayout, tooltipStyle.getTooltipStyle())
                        .setMessage(tooltipStyle.getTooltipMessage())
                        .setGravity(Gravity.BOTTOM | Gravity.START)
                        .show(true);
            }
    ...
    private void hideRecordingDetailsLayout() {
        ...
        reportIssueLayout.setVisibility(View.GONE);
    }
    ...
    private void showRecordingDetailsLayout() {
        ...
        disposables.add(userRepository.getUser()
                ...
                .subscribe(
                        user -> {
                            if (LoginManager.LOGIN_TYPE_PARTNER.equals(user.getLoginType())) {
                                reportIssueLayout.setVisibility(View.VISIBLE);
                            } else {
                                reportIssueLayout.setVisibility(View.GONE);
                            }
                        },
    ...
    private void initRecordingDetailsLayout() {
        if (!(ExtensionsKt.enableMapForRecording(appPrefs))) {
            ConstraintSet set = new ConstraintSet();
            set.clone((ConstraintLayout) fragmentLayout);
            set.connect(R.id.layout_fragment_camera_controls_recording_details, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START);
            set.applyTo((ConstraintLayout) fragmentLayout);
        }
