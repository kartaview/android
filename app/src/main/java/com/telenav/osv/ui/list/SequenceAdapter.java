    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        try {
            ...
            if (holder instanceof UserHeaderViewHolder) {
                ...
                lp.width = resources.getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT ? LinearLayout.LayoutParams.MATCH_PARENT: (int) Utils.dpToPx(activity,
                ...
                if (mIsHeaderScrollable) {
                    ...
                    width = (int) ((width - activity.getResources().getDimension(portrait ? R.dimen.sequence_list_padding_side_portrait : R.dimen.sequence_list_padding_side_landscape) * 2) / 3 * 4);
                    ...
                    int padding = (int) activity.getResources().getDimension(portrait ? R.dimen.sequence_list_padding_side_portrait : R.dimen.sequence_list_padding_side_landscape);
...
//                        .placeholder(R.drawable.custom_image_loading_background)
                        .signature(new StringSignature(sequence.getThumbLink() + " " + sequence.getLocation().getLatitude() + ", " + sequence.getLocation().getLongitude()))
                ...
                if (sequence.getAddress().equals("")) {
                    ...
                    sequenceHolder.addressTextItem.setText(sequence.getAddress().equals("") ? sequence.getLocation().getLatitude() + ", " + sequence.getLocation().getLongitude()
                            : sequence
                            .getAddress());
                }
                sequenceHolder.totalImagesTextItem.setText("" + sequence.getOriginalFrameCount());
                sequenceHolder.totalImagesTextItem.setCompoundDrawablesWithIntrinsicBounds(resources.getDrawable(R.drawable.vector_camera_gray), null, null, null);
                ...
                if (sequence instanceof UserOnlineSequence) {
                    ...
                    if (sequence.getCurrency() != null && !sequence.getCurrency().equals("") && !sequence.getServerStatus().equals(UserDataManager.SERVER_STATUS_REJECTED)) {
                        String first = Utils.formatMoney(sequence.getValue()) + "\n";
                ...
                if (sequence.getTotalLength() > 0) {
                    ...
                    sequenceHolder.totalLengthTextItem.setCompoundDrawablesWithIntrinsicBounds(resources.getDrawable(R.drawable.vector_distance_gray), null, null,
                            null);
                }
    ...
    private void reverseGeocodeAddress(final Sequence sequence, boolean retry) {
        ...
        if (SKReverseGeocoderManager.getInstance() != null) {
            SKSearchResult addr = SKReverseGeocoderManager.getInstance().reverseGeocodePosition(new SKCoordinate(sequence.getLocation().getLongitude(), sequence.getLocation()
                    .getLatitude()));
            if (addr != null) {
    ...
    public void refreshDetails(String[] acceptedDistance, String[] rejectedDistance, String[] obdDistance, String totalPhotos, String totalTracks) {
    ...
    private SpannableString getSpannableForStats(String first, String second) {
        ...
        styledString.setSpan(new TextAppearanceSpan(activity, R.style.profileHeaderInfoTextLarge), first.length(), second.length() + first.length(), 0);
        return styledString;
    }
    ...
    private void setAnimation(View viewToAnimate, int position) {
        if (position != 0) {//todo gridlayout instantiates 2 items at the same time which breaks the animation
