...
public class ByodPayRatesFragment extends DialogFragment {
    ...
    private static class PayRateItemsAdapter extends RecyclerView.Adapter<PayRateItemViewHolder> {
        ...
        public void onBindViewHolder(PayRateItemViewHolder holder, int position) {
            ...
            String previousPasses;
            if (payRateItem.payRateCoverageInterval.maxPass == 0 && payRateItem.payRateCoverageInterval.minPass == 0) {
                previousPasses = context.getString(R.string.byod_pay_rate_new_road);
            } else if (payRateItem.payRateCoverageInterval.maxPass == 0 && payRateItem.payRateCoverageInterval.minPass != 0) {
                previousPasses = Integer.toString(payRateItem.payRateCoverageInterval.minPass);
            } else {
                previousPasses = Integer.toString(payRateItem.payRateCoverageInterval.maxPass);
            }
            bindContent(holder, previousPasses,
                    payRateData.getCurrency() + FormatUtils.formatMoneyForPreferredDistanceUnit(context, payRateItem.obdPayRateValue, false),
                    payRateData.getCurrency() + FormatUtils.formatMoneyForPreferredDistanceUnit(context, payRateItem.nonObdPayRateValue, false));
        }
