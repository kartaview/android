 ...
 */
public class LocalSequenceFragment extends KVBaseFragment implements GenericListener, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        parentView = view;
        appPrefs = ((KVApplication) getActivity().getApplication()).getAppPrefs();
    ...
    public ToolbarSettings getToolbarSettings(KVToolbar kvToolbar) {
        ...
        getActivity().getWindow().setStatusBarColor(getResources().getColor(R.color.default_purple));
    ...
    private void reverseGeocodeAddress(final LocalSequence sequence, final LocalSequenceFragment.ViewHolder holder) {
        ...
        Intent reverseLocationIntent = new Intent(getContext(), IntentServiceFetchAddress.class);
        AddressResultReceiver resultReceiver = new AddressResultReceiver(new Handler(handlerThread.getLooper()));
        reverseLocationIntent.putExtra(IntentServiceFetchAddress.RECEIVER, resultReceiver);
        reverseLocationIntent.putExtra(IntentServiceFetchAddress.LOCATION_DATA_LOC, initialLocation);
        reverseLocationIntent.putExtra(IntentServiceFetchAddress.LOCATION_DATA_SEQ_ID, sequence.getID());
        reverseLocationIntent.putExtra(IntentServiceFetchAddress.LOCATION_DATA_SEQ_POS, holder.getAdapterPosition());
        getActivity().startService(reverseLocationIntent);
        sequence.getDetails().setAddressName(DEFAULT_SEQUENCE_ADDRESS);
        Activity activity = getActivity();
        if (activity != null) {
    ...
    private class SequenceListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        void updateItemAddress(String address, int position) {
            data.get(position).getDetails().setAddressName(address);
            Activity activity = getActivity();
            if (activity != null) {
    ...
    class AddressResultReceiver extends ResultReceiver {
        ...
        private String TAG = AddressResultReceiver.class.getName();
        ...
        protected void onReceiveResult(int resultCode, Bundle resultData) {
            if (resultCode == IntentServiceFetchAddress.SUCCESS_RESULT) {
                Log.d(this.TAG, "onReceiveResult. Status: geocoding success.");
                String addressResult = resultData.getString(IntentServiceFetchAddress.RESULT_DATA_KEY);
                if (!StringUtils.isEmpty(addressResult)) {
                    Log.d(this.TAG, "onReceiveResult. Status: address received successfully.");
                    String sequenceId = resultData.getString(IntentServiceFetchAddress.RESULT_DATA_SEQUENCE_ID);
                    if (!StringUtils.isEmpty(sequenceId)) {
                        Log.d(this.TAG, "onReceiveResult. Status: updating the database. Sequence id: " + sequenceId);
                        sequenceLocalDataSource.updateAddressName(sequenceId, addressResult);
                        localSequencesAdapter.updateItemAddress(addressResult, resultData.getInt(IntentServiceFetchAddress.RESULT_DATA_SEQUENCE_POSITION));
                    }
                }
            }
        }
