...
public class AnimationUtils {
    ...
    /**
     * Resizes the camera preview based on given param.
     * @param start if {@code true} this will make the camera a small preview, otherwise it will reset the values for it.
     */
    public static void resizeCameraPreview(boolean start, Activity activity, View largeContainer) {
        if (activity == null) {
            return;
        }
        Resources resources = activity.getResources();
        int cameraLayoutPreviewId = R.id.layout_activity_obd_fragment_camera_preview_container;
        ViewGroup viewGroup = activity.findViewById(cameraLayoutPreviewId);
        AnimationUtils.changeBoundsDelayedTransition(viewGroup);
        ...
        set.clone(parent);
        int newWidth = ConstraintSet.MATCH_CONSTRAINT;
        int newHeight = ConstraintSet.MATCH_CONSTRAINT;
        int previewMargin = 0;
        if (start) {
            viewGroup.bringToFront();
            largeContainer.setBackgroundColor(resources.getColor(R.color.tagging_background));
            set.clear(cameraLayoutPreviewId, ConstraintSet.TOP);
            set.clear(cameraLayoutPreviewId, ConstraintSet.END);
            previewMargin = Math.round(resources.getDimension(R.dimen.camera_preview_tagging_margin));
            int previewMarginTop = Math.round(resources.getDimension(R.dimen.camera_preview_tagging_margin_top));
            set.connect(cameraLayoutPreviewId, ConstraintSet.TOP, R.id.guidelineHorizontal, ConstraintSet.TOP, previewMarginTop);
            set.connect(cameraLayoutPreviewId, ConstraintSet.END, R.id.guidelineVertical, ConstraintSet.END, previewMargin);
            viewGroup.setClipToOutline(true);
        } else {
            largeContainer.bringToFront();
            largeContainer.setBackgroundColor(resources.getColor(R.color.default_transparent));
            set.connect(cameraLayoutPreviewId, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP, previewMargin);
            set.connect(cameraLayoutPreviewId, ConstraintSet.END, ConstraintSet.PARENT_ID, ConstraintSet.END, previewMargin);
            viewGroup.setClipToOutline(false);
        }

        set.constrainWidth(cameraLayoutPreviewId, newWidth);
        set.constrainHeight(cameraLayoutPreviewId, newHeight);
        set.connect(cameraLayoutPreviewId, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, previewMargin);
        set.connect(cameraLayoutPreviewId, ConstraintSet.BOTTOM, ConstraintSet.PARENT_ID, ConstraintSet.BOTTOM, previewMargin);
        set.applyTo(parent);
    }
