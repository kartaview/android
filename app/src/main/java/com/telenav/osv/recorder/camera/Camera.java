...
public interface Camera {
    ...
    /**
     * Opens the camera having the given id and notifies the registered subscribers about the camera initialisation status.
     * If camera is already opened, the method returns {@code complete} without trying to reopen the camera.
     *
     * @param cameraId the identifier of the camera which should be opened. The list of supported cameras can be found using {@link #getCameraDevices()}.
     * @return a {@link Completable} instance to indicate when open camera operation is completed or when an exception was encountered.
     */
    ...
    /**
     * @return {@code true} if the camera has video mode enabled, {@code false} otherwise.
     */
    boolean isVideoMode();

    /**
     * @return {@code CameraSensorData} from the current API.
     */
    void getCameraSensorDataAsync(MetadataCameraCallback metadataCameraCallback);

    /**
     * @return a list of {@code CameraDeviceData} objects representing all the supported cameras.
     */
