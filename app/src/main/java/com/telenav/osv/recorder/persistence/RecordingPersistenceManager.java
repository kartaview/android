...
public abstract class RecordingPersistenceManager implements RecordingPersistence {
    ...
    /**
     * Reference to the callback for when a photo is taken.
     */
     ...
     */
    protected RecordingPersistenceManager(SequenceLocalDataSource sequenceLocalDataSource, LocationLocalDataSource locationLocalDataSource, MetadataPhotoVideoCallback metadataPhotoVideoCallback) {
     ...
     * @param index the index representing either the video/photo index based on who will call this method
    ...
    protected boolean persistLocation(Location location, @Nullable String frameID, @Nullable String videoID, long diskSize, int index, long timestamp) {
        ...
        if (locationPersist) {
            ...
            //added a check for videoId which will not use the index since the index represents the persistence index therefore if it is photo it will be the same as the frameIndex
            metadataPhotoVideoCallback.onPhotoVideoCallback(timestamp, frameIndex, videoID == null ? 0 : index, location);
