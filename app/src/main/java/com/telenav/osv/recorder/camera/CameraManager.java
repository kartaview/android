...
public class CameraManager implements Camera {
    ...
    public CameraManager(Context context, Size pictureSize, Size screenSize, boolean isJpegMode) {
        ...
        cameraInitialization = new Camera2InitManager(context, pictureSize, screenSize, getSupportedImageFormat(isJpegMode));
        if (cameraInitialization.getHardwareLevel() == CameraMetadata.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY) {
            ...
            cameraInitialization = new Camera1InitManager(context, pictureSize, screenSize, getSupportedImageFormat(isJpegMode));
        } else {
    ...
    public List<Size> getSupportedPictureResolutions() {
        return cameraInitialization.getSupportedPictureResolutions();
    }
    ...
    @Override
    public void setImageFormat(boolean isJpegFormat) {
        cameraInitialization.setImageFormat(getSupportedImageFormat(isJpegFormat));
    }
    ...
    private int getSupportedImageFormat(boolean isJpegFormat) {
        if (isJpegFormat) {
