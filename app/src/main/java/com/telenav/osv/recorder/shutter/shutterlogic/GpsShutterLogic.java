...
public class GpsShutterLogic extends ShutterLogic {
    ...
    public void onLocationChanged(Location location) {
        ...
        if (!location.hasSpeed()) {
            ...
            double dist = previousTakenPhotoLocation.distanceTo(location);
            if (Math.abs(dist) > 0) {
                float time = (previousTakenPhotoLocation.getTime() - location.getTime()) / 1000f;
        ...
        if (location.hasAccuracy() && location.getAccuracy() < AccuracyType.ACCURACY_MEDIUM) {
            double dist = ComputingDistance
                    .distanceBetween(previousTakenPhotoLocation.getLongitude(), previousTakenPhotoLocation.getLatitude(), location.getLongitude(), location.getLatitude());
