...
public class CameraHelper {
    ...
    public static Size chooseOptimalPictureSize(SizeMap pictureSizes, int previewWidth, int previewHeight) {
        ...
        //the 2mp is the default value for the picture size
        //if no size was found with this requirement the next size grater than 2mp will be selected.
        int twoMegaPixelLimit = 2;
        Size pictureSize = sizes.first();
        for (Size size : sizes) {
            if (size.getRoundedMegaPixels() == twoMegaPixelLimit) {
                return size;
