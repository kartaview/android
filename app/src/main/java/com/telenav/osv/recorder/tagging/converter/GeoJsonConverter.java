...
public class GeoJsonConverter {
     ...
     */
    public long convert(OSVFile recordingTaggingTxtFile, OSVFile geoJsonOutputFolder) {
        featureCollection = new TaggingFeatureCollection();
        try {
            readFromFile(recordingTaggingTxtFile);
            OSVFile geoJsonFile = new OSVFile(geoJsonOutputFolder, RECORDING_TAGGING_FILE_NAME);
    ...
    private void readFromFile(OSVFile recordingTaggingTxtFile) throws IOException {
    ...
    private long writeToFile(String data, OSVFile outputFile) throws IOException {
    ...
    private void addRoadTypeFeature(double lat, double lon, String roadType) {
        if (!roadType.equals(previousRoadType)) {
            ...
            TaggingFeature taggingFeature =
                    new TaggingFeature(
                            new TaggingProperties(roadType.equals(LOG_SYMBOL_ONE_WAY) ? TaggingBase.PROPERTY_VALUE_OSC_TAGGING_ONE_WAY :
                                    TaggingBase.PROPERTY_VALUE_OSC_TAGGING_TWO_WAY),
                            featureLineString);
    ...
    private void addEventIfRequired(long timeStamp, double lat, double lon, String closedRoad, String narrowRoad, String noteLength, String note) {
        String evenType = TaggingBase.PROPERTY_VALUE_OSC_TAGGING_NOTE;
        if (!closedRoad.equals(StringUtils.EMPTY_STRING)) {
            evenType = TaggingBase.PROPERTY_VALUE_OSC_TAGGING_CLOSED_ROAD;
        }
        ...
        if (evenType.equals(TaggingBase.PROPERTY_VALUE_OSC_TAGGING_NOTE)
