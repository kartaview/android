...
public class ScoreManager implements Score, PositionMatcher.SegmentsListener, ObdManager.ObdConnectionListener {
    ...
    /**
     * Default value for score calculation when using the segment's coverage.
     */
    ...
    @Override
    public void onSegmentsReceived(KVLatLng location) {
        if (!isRecording) {
            return;
        }
    ...
    public void onPictureTaken(Location location) {
        ...
        if (sequenceId != null) {
            Segment matchedSegment = positionMatcher.onPictureTaken(new KVLatLng(location, 0));
    ...
    private void observeOnLocationUpdates() {
        ...
        compositeDisposable.add(locationService.getLocationUpdates()
                ...
                .subscribe(location -> {
                    ...
                    Segment segment = positionMatcher.onLocationChanged(new KVLatLng(location, 0));
                    if (segment != null) {
