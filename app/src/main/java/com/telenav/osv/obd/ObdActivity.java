 ...
 */
public class ObdActivity extends OSCBaseActivity {
    ...
    private void openUploadScreen() {
        ...
        OSCBaseFragment oscBaseFragment;
        if (uploadManager.isInProgress()) {
            oscBaseFragment = UploadFragment.newInstance();
        } else {
            oscBaseFragment = LocalSequenceFragment.newInstance();
        }
        ActivityUtils.addFragmentToContainer(getSupportFragmentManager(),
                oscBaseFragment,
                R.id.layout_activity_obd_fragment_container,
                false,
                oscBaseFragment.getTag());
    }
