...
class DataConverterTest {
    ...
    fun toLocationEntity() {
        val osvLocation = generateOsvLocation()
    ...
    fun toLocationEntityWithoutNullable() {
        ...
        val osvLocation = DataConverter.toOSVLocation(locationEntity)
    ...
    @Test
    fun toLocations() {
        val osvLocations = (1..10).map { generateOsvLocation() }
    ...
    fun toLocationsEmptyInput() {
        val androidLocations = DataConverter.toLocations(listOf<OSVLocation>())
    ...
    @Test
    fun toSequenceEntityWithVideoAndPaid() {
        val localSequence = generateLocalSequence(nullableFields = false, videoCompression = true, paid = true)
        ...
        assertTrue(localSequence.rewardDetails is SequenceDetailsRewardPaid)
    }
    ...
    @Test
    fun toSequenceEntityWithJpegAndPaid() {
        val localSequence = generateLocalSequence(nullableFields = false, videoCompression = false, paid = true)
    ...
    private fun assertLocationWithNonNullables(locationEntity: LocationEntity, location: OSVLocation) {
    ...
    private fun generateOsvLocation(): OSVLocation {
        return OSVLocation(
                UUID.randomUUID().toString(),
    ...
    private fun generateLocalSequence(nullableFields: Boolean, videoCompression: Boolean, paid: Boolean = false): LocalSequence {
        ...
        val sequenceDetailsLocal = SequenceDetailsLocal(
                OSVFile(UUID.randomUUID().toString()),
                random.nextLong(),
                random.nextInt(3))
        ...
        return if (nullableFields)
            LocalSequence(
                ...
                sequenceDetailsLocal,
                if (videoCompression) video else jpeg,
                if (paid) SequenceDetailsRewardPaid(
                        random.nextDouble(),
                        UUID.randomUUID().toString(),
                        (1..10).map { DriverPayRateBreakdownCoverageItem(random.nextInt(), random.nextFloat(), random.nextFloat()) })
                else SequenceDetailsRewardPoints(random.nextDouble(), UUID.randomUUID().toString(), mapOf<Int, ScoreHistory>(1 to generateScoreHistory(), 2 to generateScoreHistory(), 3 to generateScoreHistory()))
        )
    }
